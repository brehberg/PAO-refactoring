...
*&---------------------------------------------------------------------*
REPORT yy_pao_refactor_step_02.
...
CLASS lcl_customer DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    METHODS:
      amount_for
        IMPORTING io_rental TYPE REF TO lcl_rental
...
CLASS lcl_customer IMPLEMENTATION.
  ...
  METHOD statement.
    ...
    LOOP AT mt_rentals INTO DATA(lo_each).
      DATA(lv_this_amount) = amount_for( lo_each ).
    ...
    " add footer lines
    rv_result = rv_result && |Amount owed is { lv_total_amount DECIMALS = 2 }\n|.
    rv_result = rv_result && |You earned { lv_frequent_renter_points } frequent renter points|.
  ENDMETHOD.

  METHOD amount_for.
    rv_result = 0.
    CASE io_rental->movie( )->price_code( ).
      WHEN lcl_movie=>regular.
        rv_result = rv_result + 2.
        IF io_rental->days_rented( ) > 2.
          rv_result = rv_result + ( io_rental->days_rented( ) - 2 ) * '1.5'.
        ENDIF.
      WHEN lcl_movie=>new_release.
        rv_result = rv_result + io_rental->days_rented( ) * 3.
      WHEN lcl_movie=>childrens.
        rv_result = rv_result + '1.5'.
        IF io_rental->days_rented( ) > 3.
          rv_result = rv_result + ( io_rental->days_rented( ) - 3 ) * '1.5'.
        ENDIF.
    ENDCASE.
  ENDMETHOD.
